...
controller.pedido = (req, res) => {
  // Obtén los datos enviados desde el formulario en el cuerpo de la solicitud
  const { platillo, precio, cantidad, observaciones, cliente, fecha } =
    req.body;
  ...
  if (!platillo || !precio || !cantidad || !cliente || !fecha) {
    return res.status(400).json({
      success: false,
      message:
        'Faltan campos obligatorios: platillo, precio, cantidad, cliente o fecha',
    });
  }
  ...
  const newPedido = {
    ...
    cliente,
    fecha,
    estado: 'por hacer', // Estado por defecto
  };
...
controller.update_pedido = (req, res) => {
  const { platillo, cantidad, observaciones, precio, cliente, fecha, estado } =
    req.body;
  ...
  req.getConnection((err, conn) => {
    ...
    const query =
      'UPDATE pedido SET platillo = ?, precio = ?, cantidad = ?, observaciones = ?, cliente = ?, fecha = ?, estado = ? WHERE id = ?';
    const params = [
      newPedido.platillo,
      newPedido.precio,
      newPedido.cantidad,
    ...
    conn.query(query, params, (err, result) => {
      if (err) {
        ...
        res.json({ success: true, message: 'Pedido actualizado con éxito' });
      }
    });
  });
};
...
controller.chef = async (req, res) => {
  try {
    ...
    const dataPorPreparar = pedidos.filter(
      (pedido) => pedido.estado === 'preparar',
    );
    const dataPreparando = pedidos.filter(
...
controller.mesero = async (req, res) => {
  try {
    ...
    // Separar los pedidos por estado
    const dataPorentregar = pedidos.filter(
      (pedido) => pedido.estado === 'entregar',
    );
    const dataEntregado = pedidos.filter(
